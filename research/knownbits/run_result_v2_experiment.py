...
if __name__=='__main__':
    ...
    for mode in ['baseline','sign','observed']:
        log=ROOT/'results_v2'/mode/'run.log';log.parent.mkdir(parents=True,exist_ok=True)
        print('START',mode,flush=True)
        with log.open('x') as f:
            subprocess.run([sys.executable,str(ROOT/'result_v2_run.py'),'--mode',mode,'--output',str(log.parent)],
    ...
    subprocess.run([sys.executable,str(ROOT/'validate_result.py'),'--output','validation/frozen_v2'],check=True)
    subprocess.run([sys.executable,str(ROOT/'validate_result_v2.py'),'--output','validation/frozen_v2_producer'],check=True)
