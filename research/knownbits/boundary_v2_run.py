...
SEARCH_MODULES={'boundary_v2_stage_producer','semantic_choice_producer','row_observation_producer','selection_producer','observation_producer','external_producer','stage_producer','action_producer','native_producer','symbolic_producer',
...
def strict_kernel():
    ...
    import boundary_v2_stage_kernel,regular_interfaces,prove_umin,guard_kernel,observer_kernel,mask_observers
    ...
    return boundary_v2_stage_kernel
...
def worker(args):
    ...
    try:
        if args.worker=='producer':
            from boundary_v2_stage_producer import produce
            def checkpoint(r,p):
...
if __name__=='__main__':
    ...
    for case in ([a.case] if a.case else names()):
        ...
        for phase in (['replay'] if a.replay_only else ['producer','replay']):
            ...
            try:child=subprocess.run([sys.executable,str(ROOT/'boundary_v2_run.py'),'--case',case,'--mode',a.mode,'--output',str(a.output),'--worker',phase],timeout=70)
