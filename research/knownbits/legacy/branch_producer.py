...
from regular_interfaces import discover,LETTERS
...
def produce(initial,target,budgets,progress=None,split=True):
    totals={'states':0,'transition_attempts':0,'leaves':0,'branches':0,'guard_refinements':0};leaves=[]
    def visit(path,depth):
        ...
        while True:
            ...
            if len(m.initial)>budgets['states_total']-totals['states']:raise ProofFailure({'status':'initial_state_budget'})
            try:p=discover(m,budgets['states_total']-totals['states']) if m.initial else {'status':'proved_sound','states':[],'state_count':0}
            except WorkBudget:p={'status':'transition_attempt_budget'}
            finally:totals['transition_attempts']+=m.attempts
