...
import stage_kernel as native_kernel
...
def prepare(bundle,fs,entry,mode,budget):
 ...
 return dict(normalization=norm,guarded=g,guard_trace=gt,observed=o,observer_trace=ot,premasked=p,premask_trace=pt,
             action_rounds=action_rounds,symbolic_trace=st,postmask_trace=ft,final=final,final_hash=digest(final)),sr
...
def produce(name,mode,checkpoint):
 ...
 for entry in entries:
  ...
  record['components'][entry]=dict(status='prepared',blockers=blockers,native_lemmas=len(c['symbolic_trace']),action_rewrites=sum(len(x['trace']) for x in c['action_rounds']),prepare_seconds=time.perf_counter()-t)
