...
def analyze(root):
    ...
    for entry in reg['cases']:
        ...
        for route in entry['routes']:
            ...
            rows.append(row)
    baseline = {}
    for kind, candidates in [('single_summary', ['sdk_ordinary']),
                              ('consumer_service', ['once_default', 'once_ordinary']),
                              ('independent_certificates', ['each_default', 'each_ordinary'])]:
        costs = {}
        for route in candidates:
            selected = [r for r in rows if r['delivery'] == kind and r['route'] == route
                        and r['status'] == 'certified' and r['group'] not in ('assessment', 'diagnostic')]
            if selected:
                costs[route] = sum(r['lifecycle_ns']['median'] for r in selected)
        if costs:
            baseline[kind] = dict(route=min(costs, key=lambda k: (costs[k], k)), selection_scores_ns=costs,
                                  scope='this complete registered run/shard; score selects control, not cross-width speedup')
    return dict(schema='qkf-sdk-cost-contracts-summary-v1', shard=shard, cases=len(reg['cases']),
                ...
                F1='open', algorithms_changed=False)
