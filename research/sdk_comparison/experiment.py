"""Serial preregistered PR47 run; each attempt is recorded before execution."""
import argparse
...
def run(root, shard='all'):
    ...
    for index, case in enumerate(population):
        print(f"PR47 {index + 1}/{len(population)} {case['name']}", file=sys.stderr, flush=True)
        folder = root / case['name']
        folder.mkdir()
        save_json(folder / 'case.json', case)
        audits = {}
        def checked(route, output, history=False):
            command = ['research.sdk_comparison.verify', str(folder / 'case.json'), route,
        ...
        for route in case['routes']:
            ...
            def audit():
                value = child(['research.sdk_comparison.worker', str(folder / 'case.json'), route, str(output), '--audit'],
            ...
            def memory():
                value = child(['research.sdk_comparison.worker', str(folder / 'case.json'), route, str(output), '--memory'],
                              output, options['build_timeout_seconds'])
        ...
        for trial in range(options['timing_repeats']):
            for route in schedule(case['routes'], global_index, trial):
                ...
                def timing():
                    value = child(['research.sdk_comparison.worker', str(folder / 'case.json'), route, str(output)],
                                  output, options['build_timeout_seconds'])
                    require(audits[route] is not None, 'missing successful audit reference')
                    compare_identity(audits[route], value)
                    return value
        ...
        print(f"PR47 completed {case['name']}; attempts={len(attempts)} failures={len(failures)}", file=sys.stderr, flush=True)
    try:
        report = analyze(root)
    except Exception as error:
