...
DERIVATION = [
    ...
    ["signed_bucket_bridge", "caller clamps and common-prefix masks fix the sign bit whenever a helper order result is used; within one sign bucket signed order equals nonsign payload order"],
    ["descending_adapter", "setOptionalBits strong maximum supplies exact upper endpoint in that fixed sign bucket"],
    ["lower_floor_adapter", "the descending scan inside computeLowerBound supplies the greatest compatible value no greater than the lower bound within that bucket"],
    ["successor_adapter", "the checked cyclic successor plus caller-derived membership and a greater-word witness supplies the exact nonwrapping lower endpoint"],
...
def check(source, spec, certificate):
    ...
    return {
        ...
        "derived_helper_premises": [
            "caller clamp/common-prefix state fixes the sign bucket at each order-sensitive helper use",
            "descending seed is that sign-bucket mask minimum and is no greater than its bound",
            "computeLowerBound floor uses the same masks and sign-bucket minimum seed",
            "successor seed is a legal floor result in the same mask carrier",
            "floor<bound together with bound<=same-sign bucket maximum proves a greater legal word exists, so cyclic wrap is impossible",
        ],
        "stabilization": {
