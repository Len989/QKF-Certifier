...
public final class IntegerStamp extends PrimitiveStamp {
     ...
     */
    private static long computeLowerBound(int wordWidth, long floorLimit, long requiredMask, long possibleMask, boolean allowZero) {
        long resultWord = minValueForMasks(wordWidth, requiredMask, possibleMask);
        final long freeMask = possibleMask & ~requiredMask & CodeUtil.mask(wordWidth - 1);
        if (resultWord < floorLimit) {
            if (freeMask == 0) {
                resultWord = 0;
            } else {
                // First find the largest value which is less than or equal to the current bound
                for (int index = wordWidth - 1; index >= 0; index--) {
                    long cellBit = 1L << index;
                    if ((cellBit & freeMask) != 0) {
                        if (resultWord + cellBit <= floorLimit) {
                            resultWord += cellBit;
                        }
                    }
                }
                GraalError.guarantee(resultWord <= floorLimit, "should have been sufficient");
                if (resultWord < floorLimit) {
                    // Increment the first optional cellBit and then adjust the wordWidth upward until it's
                    // compatible with the masks
                    boolean started = false;
                    for (int index = 0; index < wordWidth - 1; index++) {
                        long cellBit = 1L << index;
                        if (started) {
                            // We have to propagate any carried cellBit that changed any wordWidth which must
                            // be set or cleared.
                            if ((cellBit & requiredMask) != 0 && (resultWord & cellBit) == 0) {
                                // A requiredMask cellBit has been cleared so set it
                                resultWord |= cellBit;
                            }
                            if ((cellBit & possibleMask) == 0 && (resultWord & cellBit) != 0) {
                                // A cellBit has carried into the clear section so it needs to propagate
                                // into an possibleMask cellBit.
                                resultWord += cellBit;
                            }
                        } else if ((cellBit & freeMask) != 0) {
                            resultWord += cellBit;
                            started = true;
                        }
                    }
