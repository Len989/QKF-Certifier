...
PREFIX = 'qkf_joint_carriers_2026-09-13/'
PRIOR_NAME = 'QKF_GRAAL_TRANSFER_2026-09-13.zip'
PRIOR_HASH = '61720068a6c58b5e85cd6d7d11b1374ade72a0689de417eb8719863131e65e72'
...
if __name__ == '__main__':
    ...
    p.add_argument('--output', type=Path, default=ROOT.parent / 'QKF_JOINT_CARRIERS_2026-09-13.zip')
    a = p.parse_args()
    if a.output.exists():
        ...
        with tarfile.open(temp, 'w:xz', preset=9 | lzma.PRESET_EXTREME) as tar:
            for path in paths:
                ...
                with path.open('rb') as f:
                    tar.addfile(info, f)
        temp.replace(packed)
        save(manifest, dict(schema='qkf-joint-packed-data-v1', archive=packed.name,
    ...
    save(ROOT / 'PACKAGE_MANIFEST.json', dict(schema='qkf-joint-complete-package-v1', files=package_records,
        packed_files=len(records), inherited_paper_files=len(json.loads((ROOT / 'PAPERS_INHERITANCE.json').read_text())['files']),
        history='Exact complete previous archive plus its complete earlier archives; papers recovered through two verified nested archives',
