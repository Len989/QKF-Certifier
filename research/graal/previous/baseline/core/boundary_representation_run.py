...
def prepare(e,mode):
    import mask_observers,action_producer,boundary_producer
    cur=e;phases=[]
    ...
    for _ in range(3):
        ...
        nxt,t=mask_observers.produce(cur,2);phases.append(dict(kind='mask',trace=t,after=nxt));cur=nxt
    for _ in range(3):
        nxt,t,s=boundary_producer.produce(cur,mode)
...
def produce(mode):
    ...
    save(ROOT/'results/representations'/mode/'producer.json',dict(records=records))


def replay(mode):
    from boundary_run import strict_kernel,SEARCH_MODULES
    strict_kernel()
    from boundary_cases import cases
    from qkf_certifier.kernel import digest
    from regular_interfaces import tree,order
    from word_oracle import expr_value
    import mask_observers,action_kernel,boundary_kernel
    from prefix_masks import supported
    import itertools,collections
    rows=json.loads((ROOT/'results/representations'/mode/'producer.json').read_text())['records']
    ...
    for saved,source in zip(rows,declared):
        ...
        for phase in cert['phases']:
            ...
            elif phase['kind']=='observation':boundary_kernel.replay(cur,phase['trace'],nxt)
    ...
    if a.phase=='producer':produce(a.mode)
        ...
        for m in MODES:
            for phase in ['producer','replay']:
                expected=ROOT/'results/representations'/m/('producer.json' if phase=='producer' else 'summary.json')
                if expected.exists():raise ValueError('use a fresh experiment directory')
                subprocess.run([sys.executable,str(ROOT/'boundary_representation_run.py'),'--mode',m,'--phase',phase],check=True,timeout=180)
