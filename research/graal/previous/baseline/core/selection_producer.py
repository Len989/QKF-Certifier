...
import selection_kernel as native_kernel
import action_producer,observation_producer
import producer_v5
...
def prepare(bundle,fs,entry,mode,budget):
 ...
 observation_rounds=[]
 for round_number in range(3):
  selected,trace,stats=observation_producer.produce(cur,mode)
  if not trace:break
 ...
 return dict(normalization=norm,guarded=g,guard_trace=gt,observed=o,observer_trace=ot,premasked=p,premask_trace=pt,
             action_rounds=action_rounds,observation_rounds=observation_rounds,symbolic_trace=st,postmask_trace=ft,final=final,final_hash=digest(final)),sr
