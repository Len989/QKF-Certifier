...
SEARCH_MODULES={'selection_producer','observation_producer','external_producer','stage_producer','action_producer','native_producer','symbolic_producer',
...
def strict_kernel():
    ...
    import selection_kernel,regular_interfaces,prove_umin,guard_kernel,observer_kernel,mask_observers
...
def worker(args):
    ...
    try:
        if args.worker=='producer':
            from selection_producer import produce
            def checkpoint(r,p):
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--mode',choices=['baseline','fixed','adaptive'],required=True)
    ...
    for case in ([a.case] if a.case else names()):
        ...
        for phase in (['replay'] if a.replay_only else ['producer','replay']):
            ...
            try:child=subprocess.run([sys.executable,str(ROOT/'selection_run.py'),'--case',case,'--mode',a.mode,'--output',str(a.output),'--worker',phase],timeout=70)
