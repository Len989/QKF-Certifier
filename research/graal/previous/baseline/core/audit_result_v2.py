...
def audit():
    ...
    for mode in ['baseline','sign','observed']:
        rs=read(ROOT/'results_v2'/mode/'summary.json')['records']
        ...
        for r in rs:
            ...
            pp=ROOT/'results_v2'/mode/'prepared'/(case+'.json');prepared=read(pp)
            ...
            if whole:
                ...
                cp=ROOT/'results_v2'/mode/'certificates'/(case+'.json')
                assert sha(cp)==p['certificate_sha256'] and read(cp)['components']==prepared
            else:
                assert not (ROOT/'results_v2'/mode/'certificates'/(case+'.json')).exists()
            if mode=='baseline':
        ...
        for path in (ROOT/'results_v2'/mode).rglob('*'):
            if path.is_file():output_hashes[str(path.relative_to(ROOT))]=sha(path)
    tests=read(ROOT/'validation/frozen_v2/results.json')
    ...
    normalization=read(ROOT/'validation/frozen_v2_producer/results.json')
    assert normalization==read(ROOT/'validation/development_v3_producer/results.json')
    assert normalization['status']=='passed' and normalization['cases']==6 and normalization['original_crash_reproduced']
    first_runs={m:read(ROOT/'results'/m/'summary.json')['records'] for m in modes}
    first_failures=[dict(mode=m,case=r['case'],status=r['producer']['status'],error=r['producer'].get('error'))
                    for m,rs in first_runs.items() for r in rs if 'replay' not in r]
    assert first_failures==[dict(mode='observed',case='KnownBits_AvgCeilU',status='error',error="IndexError('tuple index out of range')")]
    unchanged_first=0
    for mode,rs in first_runs.items():
        for r in rs:
            if 'replay' not in r:continue
            case=r['case']
            old=read(ROOT/'results'/mode/'prepared'/(case+'.json'))
            new=read(ROOT/'results_v2'/mode/'prepared'/(case+'.json'))
            assert {e:c['final_hash'] for e,c in old.items()}=={e:c['final_hash'] for e,c in new.items()}
            unchanged_first+=len(old)
    ...
    summary=dict(status='passed',freeze=frozen,prior_archive_sha256=PRIOR_SHA,
        ...
        mechanism_tests={**{k:tests[k] for k in keys},'producer_normalization_cases':normalization['cases'],'carrier_refusals':len(tests['refusals']),
                         'separating_inputs':len(tests['separating_inputs'])},
        unchanged_targets_contracts_and_budgets=True,strict_replay_processes=117,
        first_frozen_run=dict(failures=first_failures,strict_replay_processes=116,unchanged_available_final_expressions=unchanged_first),
