"""Bind the implementation, prior evidence and protocol before the full run."""
...
def verify():
    ...
    for n,h in json.loads((ROOT/'BASELINE_SHA256.json').read_text())['files'].items():
        if sha(ROOT/n)!=h:raise ValueError('inherited file changed: '+n)
    return dict(frozen_files=len(f['files']),baseline_files=len(json.loads((ROOT/'BASELINE_SHA256.json').read_text())['files']))
...
if __name__=='__main__':
    ...
    if a.create:
        ...
        data=dict(schema='qkf-scoped-observation-freeze-v1',utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
