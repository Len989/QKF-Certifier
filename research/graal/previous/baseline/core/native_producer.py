...
import symbolic_producer,symbolic_bridge,native_kernel
import producer_v5
...
def prepare(bundle,fs,entry,mode,budget):
 ...
 s,st,sr=budget.produce(p) if mode=='symbolic' else (p,[],dict(attempts=[],totals={}))
 final,ft=(s,[]) if mode!='symbolic' else masks(s,2)
 return dict(normalization=norm,guarded=g,guard_trace=gt,observed=o,observer_trace=ot,premasked=p,premask_trace=pt,
             symbolic_trace=st,postmask_trace=ft,final=final,final_hash=digest(final)),sr
...
def produce(name,mode,checkpoint):
 ...
 for entry in entries:
  ...
  record['components'][entry]=dict(status='prepared',blockers=blockers,native_lemmas=len(c['symbolic_trace']),prepare_seconds=time.perf_counter()-t)
