...
import result_kernel as observation_kernel
SCHEMA='qkf-original-corpus-certificate-v14-requested-result-observations'


def replay_prepared(bundle,entry,c):
 authority=observation_kernel.source_authority(bundle,entry)
 if digest(c['context_authority'])!=digest(authority):raise ValueError('component source authority mismatch')
 ...
 for r in rounds:
  changed,steps=observation_kernel.replay(p,r['trace'],r['after'],authority)
