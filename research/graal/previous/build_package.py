...
PREFIX = 'qkf_universal_upper_2026-09-13/'
PRIOR_NAME = 'QKF_JOINT_CARRIERS_2026-09-13.zip'
PRIOR_HASH = '9e27a2fb575d4717e6b6110db0fb83a9354203f6110e34affa8ec271f7535686'
...
if __name__ == '__main__':
    ...
    p.add_argument('--output', type=Path, default=ROOT.parent / 'QKF_UNIVERSAL_UPPER_2026-09-13.zip')
    a = p.parse_args()
    if a.output.exists():
        ...
        with tarfile.open(temp, 'w:xz', preset=9 | lzma.PRESET_EXTREME) as tar:
            for path in paths:
                ...
                with path.open('rb') as f:
                    tar.addfile(info, f)
        temp.replace(packed)
        save(manifest, dict(schema='qkf-universal-upper-packed-data-v1', archive=packed.name,
    ...
    save(ROOT / 'PACKAGE_MANIFEST.json', dict(schema='qkf-universal-upper-complete-package-v1', files=package_records,
        packed_files=len(records), inherited_paper_files=len(json.loads((ROOT / 'PAPERS_INHERITANCE.json').read_text())['files']),
        history='Exact complete previous archive plus its complete earlier archives; papers recovered through three verified nested archives',
