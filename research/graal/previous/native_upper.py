...
def build():
    ...
    base=(ROOT/'baseline/native/IntegerStamp.java').read_text()
    marker='if(a[0].equals("create")) {'
    extra='''if(a[0].equals("upper")) {
    ...
    for name in ['helper_13','helper_14']:
        exact=(ROOT/'baseline/native'/(name+'.inc')).read_text().rstrip('\n')
    ...
    expected=json.loads((ROOT/'baseline/native/MANIFEST.json').read_text())['compiler_sha256']
...
def evaluate(specs):
    lines=[f'upper {s["width"]} {s["upper"]} {s["must"]:x} {s["may"]:x} {int(s["can_zero"])}' for s in specs]
    command,env=java()
    p=subprocess.run(command+['-da','-cp',str(ROOT/'native/classes'),'IntegerStamp'],env=env,
    ...
    values=[]
    for line in output:
        if not line.startswith('UP '):raise ValueError('native source error: '+line)
