...
from research.ground_query.schema import PROOF_SCHEMA, require
from .ground import input_of


def prove(prepared, horizon=None):
    inp = input_of(prepared)
    ...
    active = [i for i, depth in enumerate(inp.depths) if depth <= horizon]
    ...
    if any(find(a) != find(b) for a, b in inp.queries):
        ...
        models.append({'horizon': horizon, 'domains': {s: max(1, len(v)) for s, v in classes.items()},
    ...
    for a, b in inp.queries:
        if find(a) == find(b):
            ...
            goals.append({'status': 'not_entailed' if horizon == inp.horizon else 'not_visible', 'model': 0})
    ...
    return {'schema': PROOF_SCHEMA, 'request_sha256': inp.identity, 'mode': 'entailment',
            'horizon': horizon, 'events': events, 'models': models, 'goals': goals}, {
