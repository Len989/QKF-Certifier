...
def registered_cases():
    for name, request in examples():
        yield {'name': name, 'family': 'named', 'mode': 'exact_threshold', 'horizon': None}, request
    for name, mode, horizon in (('gap_bounded', 'exact_threshold', 1),
                                ('gap_entailment', 'entailment', None)):
        yield {'name': name, 'family': 'named', 'mode': mode, 'horizon': horizon}, gap()
    for name, profile, request in catalan_cases():
        yield {'name': name, 'family': 'catalan', 'mode': 'exact_threshold', 'horizon': None,
               'expected_profile': list(profile)}, request
