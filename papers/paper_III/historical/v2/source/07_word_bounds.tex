...
For an empty carrier, the contract imposes neither mask membership nor $w$-bit representability on the returned lower value. Consumers must process the empty outcome before reusing such a value as a typed bound. Full \texttt{create} also updates temporary masks, passes through potentially incompatible states, and limits stabilization to three iterations. Its complete preservation and termination argument is the next composition task.
