...
from research.ground_query.fixtures import (
    encode,
    examples,
    gap,
    shared_example,
...
def negatives(cases):
    ...
    yield changed(
        "equation_wrong_sort", lambda r, c: r["equations"][0].__setitem__(1, 2), "typed_binary"
    )
    yield changed(
        "query_wrong_sort", lambda r, c: r["queries"][0].__setitem__(1, 2), "typed_binary"
    )
    ...
    yield changed(
        "unused_bad_event", lambda r, c: c["events"].append(dict(c["events"][0], equation=1))
    )
    ...
    for name, action in (
        ...
        (
            "extra_goal",
            lambda d: d["certificate"]["goals"].append(deepcopy(d["certificate"]["goals"][0])),
        ),
        ...
        entries.append(
            {
                "name": prefix,
                "expected": False,
                "python_rejected": request is not None,
                "decoded_only": request is None,
            }
        )
    files["Examples.lean"] = (HEADER + "\n".join(pieces) + "\nend QKFGround.Examples\n").encode()
    manifest = {
        "schema": "qkf-ground-lean-fixtures-v1",
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {"positive": result["positive_count"], "negative": result["negative_count"]},
            sort_keys=True,
        )
    )
