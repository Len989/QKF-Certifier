...
def decoded_data(request, certificate):
    ...
    fields(
        certificate,
        ("schema", "request_sha256", "mode", "horizon", "events", "models", "goals"),
        "proof",
    )
    ...
    for name in symbols:
        ...
        signature.append(
            {"args": [sorts[s] for s in spec["args"]], "result": sorts[spec["result"]]}
        )
    nodes = []
    ...
    for event in certificate["events"]:
        ...
        fields(
            event,
            ("left", "right", "depth", "rule", "equation" if rule == "axiom" else "premises"),
            "event",
        )
    ...
    for i, (q, goal) in enumerate(zip(request["queries"], certificate["goals"])):
        ...
        if goal["status"] == "equal":
            ...
            goals.append(
                {"path": [natural(e) for e in goal["path"]], "depth": natural(goal["depth"])}
            )
            indices.append(i)
    return {
        "request": {
            ...
            "signature": signature,
            "nodes": nodes,
            "equations": [pair(p) for p in request["equations"]],
            "queries": queries,
        },
        "certificate": {
            "horizon": natural(certificate["horizon"]),
            "events": events,
            "goals": goals,
        },
        "binding": {
            ...
            "sort_ids": sorts,
            "symbol_ids": symbols,
            "positive_query_indices": indices,
            "excluded_query_indices": [
                i for i in range(len(certificate["goals"])) if i not in indices
            ],
...
def render(name, data, accepted=True):
    ...
    signature = lean_list(
        "⟨" + ns(s["args"]) + ", " + n(s["result"]) + "⟩" for s in req["signature"]
    )
    nodes = lean_list("⟨" + n(s["op"]) + ", " + ns(s["args"]) + "⟩" for s in req["nodes"])

    def pairs(ps):
        return lean_list("(" + n(a) + ", " + n(b) + ")" for a, b in ps)
...
def export(request, certificate, name):
    ...
    require(
        bool(data["certificate"]["goals"]) or not request["queries"], "no positive goal to export"
    )
