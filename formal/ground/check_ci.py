...
THEOREMS = [
    ...
    "eval_typed",
    "typed_goal_sound",
    "accepted_query_typed",
    "accepted_goal_values",
]
...
def run(output, lake="lake", require_clean=False):
    ...
    try:
        ...
        names += [
            "QKFGround.Examples." + c["name"] + ("_accepted" if c["expected"] else "_rejected")
        ...
        result = {
            ...
            "positive_dags": fixture["positive_count"],
            "negative_dags": fixture["negative_count"],
            "positive_goals": sum(
                len(c.get("positive_query_indices", [])) for c in fixture["cases"]
            ),
        ...
        (output / "FAILURE.json").write_text(
            json.dumps({"error": str(error), "commands": commands}, indent=2)
        )
        raise
