...
class GroundExportTests(unittest.TestCase):
    ...
    def test_exact_terms_equations_events_and_goals_mapping(self):
        for name, (req, cert) in self.cases.items():
            with self.subTest(name=name):
                ...
                self.assertEqual([{'op': inverse[n['op']], 'args': n['args']} for n in r['nodes']], req['nodes'])
                sorts = {i: name for name, i in binding['sort_ids'].items()}
                self.assertEqual({inverse[i]: {'args': [sorts[s] for s in spec['args']],
                                              'result': sorts[spec['result']]}
                                  for i, spec in enumerate(r['signature'])}, req['signature'])
                indices = binding['positive_query_indices']
                self.assertEqual(r['queries'], [req['queries'][i] for i in indices])
                self.assertEqual(c['goals'], [{k: cert['goals'][i][k] for k in ('path', 'depth')} for i in indices])
    ...
    def test_mixed_certificate_has_explicit_positive_projection(self):
        ...
        self.assertEqual(sorted(data['binding']['positive_query_indices'] + data['binding']['excluded_query_indices']),
                         list(range(36)))
        self.assertTrue(all(cert['goals'][i]['status'] != 'equal' for i in data['binding']['excluded_query_indices']))
    ...
    def test_export_tamper_detected_by_regeneration(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(fixtures, 'ROOT', root), self.assertRaisesRegex(ValueError, 'fixture/export drift'):
    ...
    def test_lean_has_no_proof_holes_or_native_shortcut(self):
        for p in [*fixtures.ROOT.glob('*.lean'), *fixtures.ROOT.joinpath('Ground').glob('*.lean')]:
            with self.subTest(path=p.name):
                text = p.read_text()
                self.assertIsNone(re.search(r'\b(sorry|admit|native_decide|unsafe)\b|^\s*(axiom|opaque)\s', text, re.M))
    ...
    def test_preserved_accepted_engine_and_suites(self):
        ...
        self.assertEqual(result['explicit_edit_exceptions'],
